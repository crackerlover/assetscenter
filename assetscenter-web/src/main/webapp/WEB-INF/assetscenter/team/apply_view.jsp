...
<title>AssetsCenter</title>
	...
	function initDataGrid(queryString){
		if(null == queryString) {
			queryString='?applicant=&team=&role=-1&status=&startTime=&endTime=';
		}
		queryString = queryString.replace(/&amp;/g, '&')
		$('#applyView').datagrid({
			title:'Team Apply Records',  
		    width:'auto',  
		    height:'auto',  
		    remoteSort:false,  
		    singleSelect:true,  
		    ...
		    fitColumns:false, 
		    pageList:[10, 15, 20],
		    url:'applyTeamView.do' + queryString,  
		    sortName: 'APPLY_TIME',
			sortOrder: 'desc',
			idField:'id',
			frozenColumns:[[
	...
	<script type="text/javascript">
		function confirmApply() {
		    ...
		    $win = $('#confirmApply').window({
				    ...
				    collapsible:false,
				    cache:false,
				    href:'teamInitial.do?method=confirm&id='+row.id
		         });
