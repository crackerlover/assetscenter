...
<title>AssetsCenter</title>
	...
	function initDataGrid(queryString){
		if(null == queryString) {
			queryString='?selectKey=&selectValue=&owner=&type=-1&status=1&startTime=&endTime=&teamId=<%=request.getAttribute("id")%>';
		}
		queryString = queryString.replace(/&amp;/g, '&');
		$('#teamTreeView').datagrid({
			title:'Asset List',  
		    width:'auto',  
		    height:'auto',  
		    remoteSort:false,  
			...
			onLoadSuccess:function(){  
                $('#teamTreeView').datagrid('clearSelections');
            }  
		});
	}
	$(function() {
...
<body class="easyui-layout" onload="initDataGrid()">
		...
		<div class="easyui-layout" fit="true">
			...
			<form id="teamTreeForm" method="post">
			<input type="hidden" id="teamId" name="teamId" value="<%=request.getAttribute("id")%>">
			...
			<div region="center" border="false">
				<table id="teamTreeView" toolbar="#toolbar"></table>
			</div>
		</div>
	</div>
