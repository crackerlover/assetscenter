	...
	function initDataGrid(queryString){
		...
		$('#teamView').datagrid({
			...
			onLoadSuccess:function(){  
                $('#teamView').datagrid('clearSelections');
            }  
		});
	}
	function delProject() {
		...
		var row = $('#teamView').datagrid('getSelected'); 
		if(null==row) {
			$.messager.alert('Warning','You must select a row firstly.'); 
			return;
		}
		if (row){
			$.messager.confirm('Confirm','Are you sure you want to delete the [' + row.id + '] '+ row.name + ' team?',function(f){  
			    if (f){  
			    	$('#teamForm').form('submit',{
						url:'teamDelete.do?id='+row.id,
	...
	<script type="text/javascript">
		function newWindow() {
		    ...
		    $win = $('#teamAdd').window({
				    ...
				    maximizable:false,
				    collapsible:false,
				    href:'teamInitial.do?method=add'
		         });
		...
		function editProject() {
		    ...
		    $win = $('#teamEdit').window({
				    ...
				    collapsible:false,
				    cache:false,
				    href:'teamInitial.do?method=edit&id='+row.id
		         });
		...
		function apply() {
		    ...
		    $win = $('#applyTeam').window({
				    ...
				    collapsible:false,
				    cache:false,
				    href:'teamInitial.do?method=apply&id='+row.id
		         });
		...
		function addMember() {
		    ...
		    $win = $('#addMember').window({
				    ...
				    collapsible:false,
				    cache:false,
				    href:'teamInitial.do?method=addMember&id='+row.id
		         });
...
<body class="easyui-layout" onload="initDataGrid()">
		...
		<div class="easyui-layout" fit="true">
			...
			<div region="center" border="false">
				<table id="teamView" toolbar="#toolbar"></table>
			</div>
		</div>
	</div>
	<div id="teamAdd" class="easyui-window" style="width:800px;height:400px;padding:5px;" closed="true"></div>
	<div id="teamEdit" class="easyui-window" style="width:800px;height:400px;padding:5px;" closed="true"></div>
	<div id="applyTeam" class="easyui-window" style="width:800px;height:400px;padding:5px;" closed="true"></div>
